...
const findInvalidCards = cardNum => {
  ...
  for (let i = 0; i < cardNum.length; i++) {
    let currCredit = cardNum [i];
    if (!validateCred(currCredit)) {
      inValid.push(currCredit);
    }
  }
  return inValid;
}
...
const idInvalidCardCompanies = inValidFirm => {
  const companies = [];
  for (let i = 0; i < inValidFirm.length; i++) {
    switch (inValidFirm[i][0]) {
      case 3:
